...
class VxHierarchy extends HTMLElement 
{
    ...
    attach(viewer)
    {
        ...
        this.viewer.on("viewer.load", async args =>
        {
            if (args.type == "model")
            {
                let tree = await this.viewer.get("model.hierarchy", { id: viewer.document.id });
                if (tree.children)
                {
                ...
                this.viewer.on("viewpoint", (viewpoint) =>
                {
                    ...
                    if (viewpoint)
                    {
                        if (viewpoint[this.viewer.document.id])
                        {
                            var details = viewpoint[this.viewer.document.id];
                ...
                viewer.on("controller", (args) =>
                {
                    ...
                    {	
                        this.viewer.post("node.unselect", { [this.viewer.document.id]: null });
        ...
        this.viewer.on("viewer.unload", async args =>
        {
            ...
            if (args.type == "model")
            {
                ...
                this.viewer.un("controller", this);
    ...
    click(event)
    {
        if (event.target.matches("input"))
        {
            ...
            {
                this.viewer.post("node.show",  { [this.viewer.document.id] : { [div.id]: [] } });
            }
            else
            {
                this.viewer.post("node.hide",  { [this.viewer.document.id] : { [div.id]: [] } });
            }
        }
        ...
        else 
        {
            ...
            if (node)
            {
                ...
                if (selected != event.target)
                {
                    ...
                    this.viewer.post("node.select", {[this.viewer.document.id]: records });
                }
            }
        }
    }
    ...
    async dblclick(event)
    {
        ...
        if (focussed)
        {
            ...
            this.viewer.post("node.focus", { [this.viewer.document.id]: null });
        }
        ...
        if (node)
        {
            ...
            if (focussed != node)
            {
                ...
                this.viewer.post("controller.view", await this.viewer.get("node.aabb",  { [this.viewer.document.id]: JSON.parse(node.getAttribute("data-records")) } ));
                
                node.classList.toggle("selected", false);
                this.viewer.post("node.unselect", { [this.viewer.document.id]: null });
                
                this.viewer.post("node.show",  { [this.viewer.document.id] : { [node.id]: [] } });
                
                node.querySelector("input").checked = true;
                this.viewer.post("node.focus", { [this.viewer.document.id]: node.id });
            }
        }
        else 
        {
    ...
    reset()
    {
        this.viewer.post("model.reset", { id: this.viewer.document.id });
        /*
        this.viewer.post("node.focus", { [this.viewer.document.id]: null });
        this.viewer.post("node.unselect", { [this.viewer.document.id]: null });
        this.viewer.post("node.show", { [this.viewer.document.id]: null });
        */
