class View3D extends HTMLElement 
{
    ...
    constructor() 
    {
        ...
        this.viewer.on("viewer.load", async (args, custom) => 
        {
            ...
            this.viewer.post("camera", { K: args.meta.photogrammetry.camera.K });
            this.viewer.post("viewer.update", { clearColor: [0.0078,0.22,0.326,1] })		
            this.viewer.post(["controller", "target"], [
    ...
    attributeChangedCallback(name, oldValue, newValue)
    {
        //if (name == "token")
        //{
        //	this.viewer.init(newValue);
        //} 
    }
        
    setCamera(matrix)
    {
        this.viewer.post("controller", { matrix, orbit: 5 });
    }
