class View2D extends HTMLElement 
{
    ...
    constructor() 
    {
        ...
        this.viewer.on("import.create", async (args, custom) => 
        {
            ...
            {
                await this.viewer.post("camera", { id: args.id, visible: true });
            }
    ...
    async loadImage(name)
    {
        if (name != this.image)
        {
            ...
            { 
                method: 'GET', 
                headers: new Headers({
                 'Authorization': `Bearer ${decodeURIComponent(this.getAttribute("token"))}`, 
            ...
            }).then(async (response) =>
            {
                if (response.ok)
                {
                    ...
                    this.viewer.post("import.create", { id: file.path, source: { data: file.url }, type: "image", x0:this.camera.x0, y0:this.camera.y0, x1:this.camera.x1, y1: this.camera.y1, visible: false });
                }
            })
        }
    }
