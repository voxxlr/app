class VaImport extends HTMLElement 
{
    constructor()
    {
        ...
        this.dom.innerHTML = `
            ...
            <ui-dropdown-select></ui-dropdown-select>
            `;		
            
        this.viewpoint = this.dom.querySelector("ui-dropdown-select");
        ...
        { 
            for (var id in args.meta.import || {})
            {
                ...
                { 
                    method: 'POST', 
                    headers: new Headers({
                     'Authorization': `Bearer ${listing.getAttribute("key")}`, 
                ...
                }).then(async (response) =>
                {
                    ...
                    entry.vxPreview = dataset.meta.preview;
        ...
        listing.addEventListener("dragstart", (event) =>
        {
            ...
            VaImport.DIV.addEventListener("drop", async (event) => 
            {
                ...
                { 
                    method: 'POST', 
                    headers: new Headers({
                     'Authorization': `Bearer ${event.dataTransfer.getData("json/content")}`, 
                     'Content-Type': "application/json",
                     'Content-Encoding': 'gzip'
                    }),
                    body: JSON.stringify({ meta : ["name","preview","viewpoints"]})
                }).then(async (response) =>
                {
                    let dataset = await response.json();
