class VaImage extends HTMLElement 
{
    constructor()
    {
        ...
        this.dom.innerHTML = `
            ...
            <style>
                ...
                .image-list[hidden] { visibility: hidden }
                .image-list .image
                {
            ...
            <vx-file ext="jpeg,jpg,png,gif" path="/test/" style="z-index: 98">
                <ui-stack>
                    <div name="progress"><span>Drag and drop images here</span></div>
                    <div class="image-list" hidden>
        ...
        this.dom.querySelector("ui-button-group").addEventListener("down", event =>
        {
            ...
            { 
                method: 'DELETE', 
                headers: new Headers({
                    'Authorization': `Bearer ${this.upload.getAttribute("token")}` 
                })
            }); 
        ...
        this.upload.addEventListener('item-done', async (event) =>
        {
            ...
            {
                let bullet = this.addImage(await response.json());
    ...
    attach(viewer, meta)
    {
        ...
        this.upload.setAttribute("token", viewer.document.token);
