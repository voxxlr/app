...
class VxTab extends HTMLElement 
{
    constructor() 
    {
        super();
    ...
    connectedCallback() 
    {
        ...
        {
            viewer = document.querySelector(this.getAttribute("target"));
        }
        else  
        {
            viewer = this.closest("vx-viewer");
        }
    ...
    async save(object)
    {
        ...
        this.dom.querySelectorAll(":host > div").forEach(item =>
        {
            object.viewpoints.push(item.content);	
        });
        return object.viewpoints;
    }
}

customElements.define("vx-tab", VxTab);
